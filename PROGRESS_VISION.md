...
- **Transitions (In Progress 🚧):** Algorithmic, macro-level controls to program transitions and alter inner voices (e.g., injecting passing tones, suspensions, or generative behavioral personas). Operates as a dedicated mode within the Chords Pattern Tab. See `transitions_plan.md` for detailed architecture.
