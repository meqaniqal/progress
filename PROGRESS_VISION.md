...
- **Transitions (In Progress 🚧):** Algorithmic controls to program transitions and alter inner voices. Core engine, UI, independent voice sustain, and multi-note melodic flourishes (enclosures, run-ups) are complete. Upcoming: Generative personas (Restless/Lazy), rhythmic anchoring to drum patterns, hierarchical collision priority, and deep editing UI (zoom/scroll, rate sliders). Operates as a dedicated mode within the Chords Pattern Tab.
