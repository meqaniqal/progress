...
- **Global vs. Local Patterns:** The pattern editor features a master toggle for `[ Global Pattern ]` vs. `[ Local Override ]`. 
  - **Global:** Define a master 4-beat rhythm/pattern for an instrument. Variable-length chords adapt intelligently. Global changes automatically cascade to all chords, *except* those where a local override has been explicitly made for that specific instrument tab.
  - **Local:** Detach a specific chord from the global groove. In the Local view, a "Reset" button reverts the tab to inherit the Global pattern, while a "Clear" button explicitly overrides the pattern to a single, un-sliced basic block spanning the chord's duration. Tooltips explain these distinct behaviors.
...
- **Integrated Drum Machine:** The Drums tab provides a multi-row grid tailored for a 4-piece kit (Kick, Snare, Closed HH, Open HH). To maximize screen real estate, row labels are hidden. Instead, users double-click the grid to add/remove hits (which automatically auditions the specific drum sound). Users can drag hits between rows to change the drum type, triggering a re-audition. Background grid lines dynamically appear to reflect the active snapping resolution.
