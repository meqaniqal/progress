# Progress Melody Generator Audit (Current — Post-Fixes)

## Executive Summary
The three melody generators (legacy, pro, mgen) have been significantly improved. Envelope clamping was relaxed, FM and plucked-square now respect user ADSR sliders, MGEN passes `gapAfter` to synthesis, `findClosest()` accepts `maxDistance`, MGEN register was fixed (consequent phrase transposition + missing defaults), all three engines now share a consistent register (C3-C5), and the IV-chord key interpretation bug was fixed (Roman numerals now resolve relative to global `baseKey`, not per-chord `key`). Remaining issues are lower-priority pitch selection edge cases, timing quirks, and architectural gaps.
...
### Issue 2.1 (NEW): Foreshadow Notes Can Land Off-Key Due to Chord Key Misinterpretation — **FIXED**
**Location:** `melodyScheduler.js:235, 482-483, 1852`
**Status:** ✅ Resolved

**What was fixed:** When a chord has `symbol: "IV"` and `key: 53`, the code was calling `getChordNotes("IV", 53)`, which interprets "IV" as "the IV chord in the key of F" (53 % 12 = 5), returning Bb major notes `[58, 62, 65]` instead of F major `[53, 57, 60]`.

Changed all `getChordNotes(chordObj.symbol, baseChordKey, ...)` calls to `getChordNotes(chordObj.symbol, keyRoot, ...)` where `keyRoot` is the global `baseKey`. The `chordObj.key` is now only used to determine the chord's pitch position, not its key signature.

**Before:** `getChordNotes("IV", 53)` → `[58, 62, 65]` (Bb major — wrong)
**After:** `getChordNotes("IV", baseKey=48)` → `[53, 57, 60]` (F major — correct)

---

### Issue 2.1 (NEW): MGEN Consequent Phrase Transposition Drops Notes an Octave — **FIXED**
**Location:** `mgen/src/engines/PhraseEngine.js:687-694`
**Status:** ✅ Resolved

**What was fixed:** The consequent phrase generation transposed antecedent notes down a 5th (7 semitones) without any register adjustment. If antecedent notes are in C4 range (60-72), the consequent notes ended up in G3 range (53-65) — an octave too low.

Added a register floor clamp to consequent notes, ensuring they stay at or above C4 (60):
```javascript
...
**Before:** Consequent notes transposed down 5th without register adjustment, dropping below C4.

**After:** Consequent notes clamped to register floor (60), staying in valid register range.

---

### Issue 2.1 (NEW): MGEN Missing Default `registerRange` in Bridge — **FIXED**
**Location:** `mgen/src/progressBridge.js:235-238`
**Status:** ✅ Resolved

**What was fixed:** The bridge was passing `registerRange: undefined` when `melodySettings` didn't define `rangeMin`/`rangeMax`. The structural pass then used defaults of `minPitch = 48` (C3) and `maxPitch = 72` (C5), causing notes to stay in C3 range.

Changed to always pass `registerRange` with defaults matching professional mode:
```javascript
registerRange: {
...
**Before:** `registerRange` was `undefined`, structural pass used `minPitch = 48` (C3) default.

**After:** `registerRange` always set to `{ min: 48, max: 72 }`, matching professional mode defaults.

---

### Issue 2.1 (NEW): Legacy Engine Register Was One Octave Too High — **FIXED**
**Location:** `melodyScheduler.js:219-226`
**Status:** ✅ Resolved

**What was fixed:** The legacy engine was generating melodies in G4-F#5 (67-79), centered around C5 (72). This was designed assuming chords were in a lower register and the melody should sit above them. With chords in C3-C4 range, the melody was an octave too high.

Lowered the legacy engine's register to match MGEN/professional:
- Melody: 55-67 (E3-F#4), centered ~61 (was 67-79, centered ~72)
- Countermelody: 45-57 (C#3-E4), centered ~51 (was 57-69, centered ~63)

**Before:** Legacy melody centered at C5 (72), one octave above MGEN/professional.

**After:** All three engines now share roughly the same register (C3-C5 range).

---

### Issue 2.1 (NEW): Professional Engine Register Was One Octave Too High — **FIXED**
**Location:** `professionalMelodyScheduler.js:59-60, 224-225`
**Status:** ✅ Resolved

**What was fixed:** Professional mode defaults were `rangeMin = 60` (C4) and `rangeMax = 84` (C6), centered ~72. This was one octave above the legacy engine's new range (55-67) and MGEN's new range (48-72).

Lowered professional mode defaults to match:
```javascript
const rangeMin = settings.rangeMin || 48;
const rangeMax = settings.rangeMax || 72;
```

**Before:** Professional mode centered at C5 (72), one octave above legacy/MGEN.
...
## Register Alignment Summary (This Session)

| Engine | Before | After | Center |
|---|---|---|---|
| Legacy | 67-79 (G4-F#5) | 55-67 (E3-F#4) | ~61 |
| MGEN | 60-84 (C4-C6) | 48-72 (C3-C5) | ~60 |
| Professional | 60-84 (C4-C6) | 48-72 (C3-C5) | ~60 |

All three engines now share roughly the same register (C3-C5 range). The legacy engine's range (55-67) is a subset of MGEN/professional's range (48-72).

## IV-Chord Key Interpretation Fix (This Session)

**Bug:** When a chord has `symbol: "IV"` and `key: 53`, `getChordNotes("IV", 53)` interprets "IV" as "the IV chord in the key of F" (53 % 12 = 5), returning Bb major `[58, 62, 65]` instead of F major `[53, 57, 60]`.

**Fix:** Changed all `getChordNotes(chordObj.symbol, baseChordKey, ...)` calls to `getChordNotes(chordObj.symbol, keyRoot, ...)` where `keyRoot` is the global `baseKey`. The `chordObj.key` is now only used to determine the chord's pitch position, not its key signature.

**Files changed:**
- `melodyScheduler.js:235` — uses `keyRoot` instead of `baseChordKey` for `getChordNotes`
- `melodyScheduler.js:482-483` — uses `keyRoot` instead of `chordObj.key` for prev/next chord notes
- `melodyScheduler.js:1852` — uses `keyRoot` instead of `chordObj.key` for next chord notes
