...
export function scheduleMelody(
    ...
    // Register boundaries (Melody centered around C4 [60], Countermelody centered around E3 [52])
    const melodyRangeStart = 55;
    let melodyRangeEnd = 67;
    if (narrativeState.lowRegisterBars > 3) {
        melodyRangeEnd += 5; // push register ceiling up
    }
    const counterRangeStart = 45;
    const counterRangeEnd = 57;
    ...
    const pcOffsets = {};
    const standardNotes = getChordNotes(chordObj.symbol, keyRoot, divisions) || [];
    ...
    const prevChordNotes = prevChordObj ? (getChordNotes(prevChordObj, keyRoot, divisions) || []) : [];
    const nextChordNotes = nextChordObj ? (getChordNotes(nextChordObj, keyRoot, divisions) || []) : [];
            ...
            if (melodyPlays) {
                ...
                if (g.clusterRole === 'foreshadow_note' && settings.genre !== 'none') {
                    ...
                    if (commonScalePitches.length > 0) {
                        pitch = findClosest(gluePitch, commonScalePitches);
                    } else {
                        const nextChordRoot = nextChordObj && nextChordObj.key !== undefined ? Number(nextChordObj.key) : keyRoot;
                        let rootTarget = nextChordRoot;
                        while (rootTarget < melodyRangeStart) rootTarget += periodSize;
                        while (rootTarget > melodyRangeEnd) rootTarget -= periodSize;
                        pitch = findClosest(rootTarget, effectiveValidPitches);
                    }
    ...
    if (melodyScheduled.length > 0) {
        ...
        for (let i = 0; i < melodyScheduled.length; i++) {
            ...
            // Avoid hijacking downbeats or early notes; only apply if the note is late in the slot (within 1 beat of the next chord transition)
            ...
            if (silenceDuration > 0.8 * beatLen && !isConsequentFinal && isCloseToNextChord && (n.sixteenthStep % 4 === 0)) {
                if (nextChordObj && nextChordNotes.length > 0) {
                    ...
                    const prevActiveNote = (i > 0) ? melodyScheduled[i - 1] : null;
                    const isStepwiseFromPrev = !prevActiveNote || Math.abs(target - prevActiveNote.pitch) <= 2.01;
                    
                    if (isOrigChordTone && Math.abs(target - n.pitch) <= 3.01 && isStepwiseFromPrev) {
                        n.pitch = target;
                    }
                } else {
    ...
    melodyScheduled.forEach((n, idx) => {
        ...
        playToneFn(midiToFreq(pitch), n.stepTime, n.noteDuration, n.melodyInst, 'melody', 0, 1.0, { gapAfter, step: n.step, isAnchor1Step: n.isAnchor1Step, isAnchor2Step: n.isAnchor2Step });
