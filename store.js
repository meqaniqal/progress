...
export function setGlobalTuning(divisions, customTuning = null) {
    saveHistoryState();
    state.divisions = divisions;
    state.customTuning = customTuning;
    
    // Update all chords in the progression to match the new global tuning
    state.currentProgression.forEach(chord => {
        // If the chord has user-adjusted/manual customNotes (not autoGenerated), do not touch it!
        if (chord.customNotes && !chord.autoGenerated) {
            return;
        }

        const tuning = getEffectiveTuning(chord.symbol, divisions, customTuning);
        const isMicrotonal = divisions !== 12 || (tuning && Math.abs(tuning.periodSize - 12.0) > 0.01);
        
        if (isMicrotonal) {
            const computedNotes = getChordNotes(chord.symbol, chord.key, divisions, customTuning, true);
            if (computedNotes) {
                chord.customNotes = computedNotes.map(p => ({ pitch: p, isMicrotonal: true }));
                chord.divisions = divisions;
                chord.autoGenerated = true;
            }
        } else {
            // Back to 12-TET: Clear auto-generated microtonal notes
            if (chord.autoGenerated) {
                delete chord.customNotes;
                delete chord.divisions;
                delete chord.autoGenerated;
            } else {
                delete chord.divisions;
            }
        }
    ...
    // Update active temporary swaps as well
    Object.keys(state.temporarySwaps).forEach(index => {
        const swap = state.temporarySwaps[index];
        if (swap) {
            // If the swap has user-adjusted/manual customNotes (not autoGenerated), do not touch it!
            if (swap.customNotes && !swap.autoGenerated) {
                return;
            }

            const tuning = getEffectiveTuning(swap.symbol, divisions, customTuning);
            const isMicrotonal = divisions !== 12 || (tuning && Math.abs(tuning.periodSize - 12.0) > 0.01);
            if (isMicrotonal) {
                const computedNotes = getChordNotes(swap.symbol, swap.key !== undefined ? swap.key : state.baseKey, divisions, customTuning, true);
                if (computedNotes) {
                    swap.customNotes = computedNotes.map(p => ({ pitch: p, isMicrotonal: true }));
                    swap.divisions = divisions;
                    swap.autoGenerated = true;
                }
            } else {
                if (swap.autoGenerated) {
                    delete swap.customNotes;
                    delete swap.divisions;
                    delete swap.autoGenerated;
                } else {
                    delete swap.divisions;
                }
            }
